...
export default function QuoteForm () {
    ...
    return (
        <>
            <QuoteConfirmation isOpen={isConfirmationOpen} setIsOpen={setConfirmationOpen}/>
            <form onSubmit={handleSubmit} action="" className="bg-white rounded-3xl lg:rounded-[3rem] py-6 lg:py-12 px-4 lg:px-8 w-full max-w-[25rem] lg:max-w-none mx-auto lg:mx-0 lg:w-fit shadow-sm lg:shadow-lg z-10 relative lg:h-fit">
                <div className="flex flex-col lg:flex-row gap-2 lg:gap-8 w-full lg:w-fit">
                    <div className="flex flex-col gap-2 lg:gap-4 lg:flex-1">
                        <input required name="moving-from" type="text" className="bg-transparent rounded-3xl border border-black py-2 lg:py-4 px-4 lg:px-8 placeholder:font-normal placeholder:text-opacity-50 lg:h-[3.25rem] focus:outline-black" placeholder="Moving From *" value={collection ? collection : undefined} onChange={(e) => {setCollection(e.currentTarget.value)}}/>
                        <input required name="moving-to" type="text" className="bg-transparent rounded-3xl border border-black py-2 lg:py-4 px-4 lg:px-8 placeholder:font-normal placeholder:text-opacity-50 lg:h-[3.25rem] focus:outline-black" placeholder="Moving To *" value={moving ? moving : undefined} onChange={(e) => {setMoving(e.currentTarget.value)}}/>
                        <SizeSelect />
                    </div>

                    <div className="flex flex-col gap-2 lg:gap-4 lg:flex-1">
                        <DatePicker />
                        <input required name="name" type="text" className="bg-transparent rounded-3xl border border-black py-2 lg:py-4 px-4 lg:px-8 placeholder:font-normal placeholder:text-opacity-50 lg:h-[3.25rem] focus:outline-black" placeholder="Name *"/>
                        <input required name="email" type="email" className="bg-transparent rounded-3xl border border-black py-2 lg:py-4 px-4 lg:px-8 placeholder:font-normal placeholder:text-opacity-50 lg:h-[3.25rem] focus:outline-black" placeholder="Email *"/>
                    </div>
                </div>
