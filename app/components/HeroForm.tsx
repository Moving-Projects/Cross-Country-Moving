...
export default function HeroForm () {
    ...
    return (
        <form onSubmit={(e) => {e.preventDefault(); router.push(`/contacts?collection=${collection}&moving=${moving}`)}} className="bg-white rounded-3xl lg:rounded-[3rem] py-8 lg:py-12 px-6 lg:px-8 flex flex-col w-full lg:w-[33rem] shadow-sm lg:shadow-lg max-w-[25rem] lg:max-w-none mx-auto lg:mx-0">
            <input type="text" className="font-medium bg-transparent rounded-3xl border border-black py-2 lg:py-4 pl-4 lg:pl-8 placeholder:font-light placeholder:text-opacity-50 mb-2 lg:mb-4 lg:h-[3.25rem]" placeholder="Collection Address" value={collection} onChange={(e) => {setCollection(e.currentTarget.value)}}/>
            <input type="text" className="font-medium bg-transparent rounded-3xl border border-black py-2 lg:py-4 pl-4 lg:pl-8 placeholder:font-light placeholder:text-opacity-50 mb-6 lg:mb-8 lg:h-[3.25rem]" placeholder="Moving to Address" value={moving} onChange={(e) => {setMoving(e.currentTarget.value)}}/>
